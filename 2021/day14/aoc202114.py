...
def expand_polymer(polymer, rules):
    ...
    expanded_pairs = [polymer_pair[0] + rules[polymer_pair] + polymer_pair[1] for polymer_pair in polymer_pairs]
    # when stitching the expanded fragments together, there is overlap between the first triple's last and the second triple's first element!
...
def iterate(data, cycles):
    polymer = data[0]
    rules = data[1]
    polymer_pairs = collections.Counter([polymer[i - 1:i + 1] for i in range(1, len(polymer))])
    ...
    # we're counting double here due to the overlap, except for the begin and end elements of the original string - hence ceil!
    return math.ceil(max(count_elements.values())/2) - math.ceil(min(count_elements.values())/2)
...
def part2(data):
    """Solve part 2.
      ...
      For 40 iterations this would entail:   20.890.720.927.745 ... Oops
        ...
        When starting with a polymer of length 4 having 3 pairs, this expands into a polymer of length 7 having 6 pairs
    """
