...
this information in a different format! After the list of springs for a given
...
roups of one, one, and three broken springs (in that order) can appear
in that row: the first three unknown springs must be broken, then operational,
...
and broken springs that meet the given criteria. What is the
sum of those counts?
...
def generate_combinations(pattern: str, replacements='#.'):
    # https://stackoverflow.com/questions/69792413/replace-in-a-string-with-a-set-of-characters-give-all-possible-string-combi
    for p in map(iter, product(replacements, repeat=pattern.count('?'))):
        yield ''.join(c if c != '?' else next(p) for c in pattern)


def patterns_matching_counts(pattern: str, counts: list[int]) -> int:
    return sum([1
                for repl in generate_combinations(pattern)
                if [len(x) for x in re.findall("#+", repl)] == counts])


def part1(fname: str) -> int:
    parsed = []
    for line in u.read_file(fname):
        pattern, c = line.split()[:2]
        counts = [int(x) for x in c.split(',')]
        parsed.append((pattern, counts))

    result = 0
    for p, c in parsed:
        result += patterns_matching_counts(p, c)
    return result


@cache
def count_possibilities(springs: str, groups: tuple[int,]) -> int:
    """Recursive counting function; return 1 for possible arrangements,
       0 as soon as it becomes impossible.
       Requires caching to replace repeated invocations by lookups
    """
    # early return conditions
    if len(springs) == 0:
        # no more springs - is success if no more groups exist!
        return 1 if len(groups) == 0 else 0
    if len(groups) == 0:
        # no more groups - success if no more active springs are present
        return 0 if "#" in springs else 1
    if sum(groups) > len(springs) - springs.count("."):
        # more springs requested than options possible
        return 0

    result = 0

    if springs[0] in "?.":              # option 1 - current = inactive spring
        result += count_possibilities(springs[1:], groups)

    if springs[0] in "?#":              # option 2 - current = active spring
        current_group_size, other_groups = groups[0], groups[1:]
        if (len(springs) >= current_group_size and
            "." not in springs[:current_group_size] and
            (len(springs) == current_group_size or
                springs[current_group_size] != "#")):
            result += count_possibilities(
                    springs[current_group_size+1:], other_groups)

    return result


def part1a(fname: str) -> int:
    """Part 2 needs a different approach, folding the maps and counts.
    The regex does not scale too well to this extension.
    Recursion with memoization could prove useful here.
    """
    possibilities = 0
    for line in u.read_file(fname):
        spring_pattern, numbers = line.split()
        # needs to be hashable for memoization
        spring_counts = tuple(map(int, numbers.split(",")))
        possibilities += count_possibilities(spring_pattern, spring_counts)
...
if __name__ == "__main__":
    #print(f"Result for part1 : {part1("day12.txt")}")
    print(f"Result for part1 : {part1a("day12.txt")}")
    print(f"Result for part2 : {part2("day12.txt")}")
