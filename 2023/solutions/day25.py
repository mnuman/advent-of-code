...
communication modules: Error 2023. It doesn't say what Error 2023 means, but it
...
have too many components plugged in. Try unplugging some components and--" You
...
xyz: abc both represent the same configuration. Each connection between two
components is represented only once, so some components might only ever appear
on the left or right side of a colon.
...
two separate groups. What do you get if you multiply the sizes of these two
groups together?
...
You don't have enough stars to restart the system, though. You need 4 more.
"""
...
def parse_line(line):
    n, c = line.split(":")
    node = n.strip()
    connected = [x for x in c.split()]
    return node, connected


def construct_graph(fname):
    g = nx.Graph()
    for node, connected in [parse_line(line) for line in u.read_file(fname)]:
        g.add_edges_from([(node, c) for c in connected])
    return g


def part1(fname: str) -> int:
    g = construct_graph(fname)
    # we need three cuts to disconnect; hence, we're looking
    # for k_edge_components with k=4
    return reduce(mul, [len(c) for c in nx.k_edge_components(g, 4)])
...
if __name__ == "__main__":
    ...
    #  400_000_000_000_000)}"
    # )
    print(f"Results part1: {part1('day25.txt')}")
    print(f"Results part2: {part2('day25.txt')}")
