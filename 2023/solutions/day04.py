...
would know. He's on a different island, though - er, the small kind 
surrounded by water, not the floating kind. We really need to come up with 
a better naming scheme. Tell you what: if you can help me with something 
...
def parse_line(line: str) -> Tuple[list[int], list[int]]:
    ...
    return (
        [int(c) for c in winners.strip(" ").split(" ") if c != ""],
        [int(c) for c in my_numbers.strip(" ").split(" ") if c != ""]
    )


def count_winners(winners: list[int], my_numbers: list[int]) -> int:
    return sum(1 if n in winners else 0 for n in my_numbers)


def calculate_score(winners: list[int], my_numbers: list[int]) -> int:
    matches = count_winners(winners, my_numbers)
...
def part1(fname: str) -> int:
    ...
    for line in u.read_file(fname):
        winners, my_numbers = parse_line(line)
        result += calculate_score(winners, my_numbers)
    return result
...
def part2(fname: str) -> int:
    ...
    for idx, line in enumerate(lines):
        winners, my_numbers = parse_line(line)
        for i in range(count_winners(winners, my_numbers)):
            extras_index: int = idx + i + 1
            if extras_index < len(lines):
                card_instances[extras_index] += card_instances[idx]
    return sum(card_instances)


if __name__ == "__main__":
    print(f"Result for part1 : {part1("day04.txt")}")
    print(f"Result for part2 : {part2("day04.txt")}")
