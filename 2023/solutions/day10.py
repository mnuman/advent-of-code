...
The pipes are arranged in a two-dimensional grid of tiles:
    ...
    | is a vertical pipe connecting north and south. - is a horizontal pipe
    connecting east and west. L is a 90-degree bend connecting north and east. J
    is a 90-degree bend connecting north and west. 7 is a 90-degree bend
    connecting south and west. F is a 90-degree bend connecting south and east.
    . is ground; there is no pipe in this tile. S is the starting position of
    the animal; there is a pipe on this tile, but your sketch doesn't show what
    shape the pipe has.
...
position? Your puzzle answer was 6820.
...
.S-------7.
.|F-----7|.
.||OOOOO||. .||OOOOO||. .|L-7OF-J|. .|II|O|II|. .L--JOL--J. .....O.....
...
OF----7F7F7F7F-7OOOO O|F--7||||||||FJOOOO O||OFJ||||||||L7OOOO
...
class Grid:
    ...
    def __init__(self, lines):
        ...
        self.connections: dict[tuple[int, int], set[tuple[int, int]]] = defaultdict(set)
        self._parse_input()
    ...
    def _parse_input(self):
        for y, line in enumerate(self._lines):
            for x, symbol in enumerate(line):
                self.process_symbol(x, y, symbol)
        # self.remove_unidirectional_connections()
        self.add_source_connections()

    def add_source_connections(self):
        self.connections[self.start] = set(
            t for t in self.connections.keys() if self.start in self.connections[t]
            )

    def remove_unidirectional_connections(self):
        """
        Remove all non bidirectional connections -
        unless one of the nodes is the start node.
        """
        birectionals = {
            source: set(
                [target for target in targets
                 if source == self.start or target == self.start or source in
                 self.connections.get(target, list())
                 ]
                 ) for source, targets in self.connections.items()
                 }
        self.connections = birectionals

    def process_symbol(self, x: int, y: int, symbol: str):
        match symbol:
            case ".":
                return
            case "S":
                ...
                return
            case _:
                self.add_connection(x, y, symbol)
                return

    def add_connection(self, x: int, y: int, symbol: str):
        for offset in CONNECTION_OFFSETS[symbol]:
            connected = Grid.pos(x, y, offset)
            if 0 <= connected[0] < self._size and 0 <= connected[1] < self._size:
                self.connections[(x, y)].add(connected)

    def find_loop(self, farthest, distances):
        r = {}
        for cell, d in distances.items():
            if d in r:
                r[d].add(cell)
            else:
                r[d] = {cell}
        ...
        path = {farthest}
        current_dist = distances[farthest] - 1
        while current_dist >= 0:
            for candidate in r[current_dist]:
                if self.connections[candidate] not in path:
                    path.add(candidate)
            current_dist -= 1
        return path

    @staticmethod
    def pos(x: int, y: int, offset) -> tuple[int, int]:
        return x + offset.value[0], y + offset.value[1]


def bfs_distances(g: Grid) -> dict[tuple[int, int], int]:
    fringe: Queue[tuple[int, int]] = Queue()
    fringe.put(g.start)
    steps: dict[tuple[int, int], int] = {g.start: 0}
    visited: Set[tuple[int, int]] = set()

    while not fringe.empty():
        curr: tuple[int, int] = fringe.get()
        visited.add(curr)
        for connected in g.connections[curr]:
            if connected not in visited:
                fringe.put(connected)
            steps[connected] = min(steps.get(connected, maxsize), 1 + steps[curr])

    return steps


def part1(fname: str) -> int:
    grid = Grid(u.read_file(fname))
    bfs_dist = bfs_distances(grid)
    return max(bfs_dist.values())


def count_inside(grid, lus) -> int:
    area = 0
    for y, row in enumerate(grid.get_lines()):
        is_inside: bool = False
        for x, c in enumerate(row):
            if c in ("SF7|") and (x, y) in lus:
                is_inside = not is_inside
            else:
                if is_inside and not (x, y) in lus:
                    area += 1
...
def part2(fname: str):
    grid = Grid(u.read_file(fname))
    distances = bfs_distances(grid)
    farthest = sorted(distances, key=lambda k: distances[k])[-1]
    lus = grid.find_loop(farthest, distances)
    return count_inside(grid, lus)


if __name__ == "__main__":
    print(f"Result for part1 : {part1("day10.txt")}")
    print(f"Result for part2 : {part2("day10.txt")}")
