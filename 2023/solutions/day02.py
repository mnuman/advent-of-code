...
reaches the surface of a large island floating in the sky. You gently land in
a fluffy pile of leaves. It's quite cold, but you don't see much snow. An Elf
...
by a semicolon-separated list of subsets of cubes that were revealed
...
only 12 red cubes, 13 green cubes, and 14 blue cubes. What is the sum of
...
MAXCOUNTS: dict[str, int] = {"red": 12, "blue": 14, "green": 13}


def parse_line(line: str) -> Tuple[int, list[dict[str, int]]]:
    game, counts = line.split(":")
    game_id = int(re.findall(r"\d+", game)[0])  # strip out the game identification
    result = []
    for turn in counts.split(";"):  # turns are separated by semicolons
        matches = re.finditer(r"(?P<number>\d+) (?P<colour>\w+)", turn)
        result.append({m.group("colour"): int(m.group("number")) for m in matches})
    return game_id, result


def is_possible(turns: list[dict[str, int]]) -> bool:
    possible: list[bool] = [
        colour in MAXCOUNTS.keys() and MAXCOUNTS[colour] >= count
        for turn in turns
        for colour, count in turn.items()]
    return all(possible)


def power_cubeset(turns: list[dict[str, int]]) -> int:
    mincubes = {}
    for turn in turns:
        for colour, number in turn.items():
            mincubes[colour] = max(mincubes.get(colour, 0), number)
    return reduce(mul, mincubes.values(), 1)


def part1(fname: str) -> int:
    result = 0
    for line in u.read_file(fname):
        game_id, turns = parse_line(line)
        if is_possible(turns):
            result += game_id
    return result
...
def part2(fname: str) -> int:
    return sum(
        power_cubeset(turns)
        for _, turns in [parse_line(line) for line in u.read_file(fname)]
        )


if __name__ == "__main__":
    print(f"Result for part1 : {part1("day02.txt")}")
    print(f"Result for part2 : {part2("day02.txt")}")
