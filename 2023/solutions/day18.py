...
large backlog of parts requests, the factory will also need a large supply
of lava for a while; the Elves have already started creating a large lagoon
...
so if "up" were north, then "right" would be east, and so on. Each trench is
also listed with the color that the edge of the trench should be painted as
...
the edge of the lagoon; the next step is to dig out the interior so that it is
...
fix the bug; one of them asks if you can extract the correct instructions
from the hexadecimal codes.

Each hexadecimal code is six hexadecimal digits long. The first five
hexadecimal digits encode the distance in meters as a five-digit hexadecimal number.
The last hexadecimal digit encodes the direction to dig: 0 means R, 1 means D,
2 means L, and 3 means U.

So, in the above example, the hexadecimal codes can be
converted into the true instructions:
...
# treat grid as y=rows --> x=cols downwards
OFFSETS = {"R": (0, 1), "L": (0, -1), "U": (-1, 0), "D": (1, 0)}


def process(instructions) -> int:
    x, y = 0, 0
    edges = [(x, y)]
    boundary_points = 0
    for i in instructions:
        qty = int(i[1])
        drow, dcol = dx, dy = OFFSETS[i[0]]
        boundary_points += qty
        x, y = x + qty * dx, y + qty * dy
        edges.append((x, y))

    shoelace = (
        abs(sum(c[0] * n[1] - c[1] * n[0] for c, n in zip(edges, edges[1:]))) // 2
    )
    # inner_points = shoelace - boundary_points // 2 + 1 (Pick's theorem)
    # so total = inner + boundary_points = shoelace + boundary_points // 2 + 1

    return shoelace + boundary_points // 2 + 1


# shoelace formula + pick's theorem
def part1(fname: str) -> int:
    instructions = [line.split() for line in u.read_file(fname)]
    return process(instructions)


def parse_line(line):
    _, _, c = line.split()[:3]
    return "RDLU"[int(c[7])], int(c[2:7], base=16)


def part2(fname: str) -> int:
    instructions = [parse_line(line) for line in u.read_file(fname)]
    return process(instructions)


if __name__ == "__main__":
    print(f"Resultaten deel1 : {part1('day18.txt')}")
    print(f"Resultaten deel2 : {part2('day18.txt')}")
