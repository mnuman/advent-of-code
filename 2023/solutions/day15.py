...
wearing goggles and a loose-fitting hard hat. The reindeer leads you to a
shelf of goggles and hard hats (you quickly find some that fit) and then
further into the facility. At one point, you pass a button with a faint
...
outside. As you admire the large assortment of lenses available to further f
ocus the light, the reindeer brings you a book titled "Initialization Manual".
...
here. Within each box, lenses are listed from front to back; each lens is shown
as its label and focal length in square brackets.

To confirm that all of the lenses are installed correctly, add up the focusing power
of all of the lenses. The focusing power of a single lens is the result of
...
class Lens:
    label: str
    strength: int = field(compare=False, hash=False, default=0)
...
def focus_power(lenses: dict[int, list[Lens]]) -> int:
    return sum(
        (k+1) * idx * lens.strength
        for k in lenses.keys()
        for idx, lens in enumerate(lenses[k], start=1)
        )


def process_tokens(tokens) -> int:
    lenses: dict[int, list[Lens]] = defaultdict(list)
    for t in tokens:
        adding_lens = "=" in t
        if adding_lens:
            label, value = t.split("=")
        else:
            label, value = t.split("-")[0], "0"
        lens = Lens(label, int(value))
        k = token_value(label)
        if k in lenses:
            lens_list = lenses[k]
            if lens in lens_list:
                if adding_lens:
                    lens_list[lens_list.index(lens)] = lens
                else:
            ...
            if adding_lens:
                lenses[k].append(lens)
    return focus_power(lenses)


def part1(fname: str) -> int:
    tokens = u.read_file(fname, separator=",")
    return sum([token_value(token) for token in tokens])
...
if __name__ == "__main__":
    print(f"Result for part1 : {part1("day15.txt")}")
    print(f"Result for part2 : {part2("day15.txt")}")
