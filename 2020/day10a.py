...
def device_joltage(joltage_list):
    return max(joltage_list) + 3


def sort_adapters(joltage_list):
    return sorted(joltage_list)


def diffs(joltage_list):
    """Must use *all* adapters ... just sort the list!"""
    diff_1 = 1 if joltage_list[0] == 1 else 0  # diff between socket (0) and first adapter
    # diff last-device always 3, but socket-first may also be 3
    diff_3 = 2 if joltage_list[0] == 3 else 1
...
if __name__ == "__main__":
    mylist = sort_adapters(utils.read_file("data/day10.txt", convert=int))
    d = diffs(mylist)
    print(f"# diff-1s: {d[0]}, diff-3s: {d[1]}, result = {d[0] * d[1]}")
