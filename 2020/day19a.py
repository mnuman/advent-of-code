...
0. In the above example, ababbb and abbbab match, but bababa, aaabbb,
and aaaabbb do not, producing the answer 2. The whole message must match all
of rule 0; there can't be extra unmatched characters in the message. (For
...
def read(filename):
    ...
    while len(lines[idx]) > 0:
        rule_number, rule_spec = lines[idx].split(': ')
        rule_alternatives = rule_spec.split(' | ')
        rule = []
        for a in rule_alternatives:
            rule.append([int(token)
                         if token[0] in '0123456789' else token.strip('"')
