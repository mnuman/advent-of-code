...
def determine_schedule(earliest_departure, busdepartures):
    buses = [int(x) for x in busdepartures.split(',') if x != 'x']
    departures = {bus * math.ceil(earliest_departure / bus): bus for bus in
                  buses}
    the_bus_departure = min(departures.keys())
    return the_bus_departure, departures[the_bus_departure]


def calc_product(earliest_departure, buses):
    earliest = determine_schedule(earliest_departure, buses)
    return (earliest[0] - earliest_departure) * earliest[1]


if __name__ == "__main__":
    TIMESTAMP = 1002576
    BUSES = "13,x,x,x,x,x,x,37,x,x,x,x,x,449,x,29,x,x,x,x," \
            "x,x,x,x,x,x,x,x,x,x,x,x,19,x,x,x,23,x,x,x,x," \
            "x,x,x,773,x,x,x,x,x,x,x,x,x,41,x,x,x,x,x,x,17"
    print(calc_product(TIMESTAMP, BUSES))
