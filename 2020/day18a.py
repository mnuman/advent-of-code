...
finds the sum of the numbers on both sides of the operator,
...
def parse_input(s):
    chars = [c for c in s.replace(' ', '')]
    idx = 0
    while idx < len(chars):
        if idx + 1 < len(chars) and chars[idx][-1] in NUMBERS and chars[
            idx + 1] in NUMBERS:
            chars[idx] = chars[idx] + chars[idx + 1]
            chars.pop(idx + 1)
        else:
            idx += 1
    return [t if t in '*+/-()' else int(t) for t in chars]
