...
def parse_xmas_list(filename):
    # read the lines of a file and return a list of integer values
    return utils.read_file(filename, convert=int)


def first_non_sum(xm_list, window_size=25, combination_size=2):
    """ Given a window_size in xmas_list, which number is the first
        not to be the sum of a combination of the previous window_size numbers?

    """
    for i in range(window_size, len(xm_list)):
        target = xm_list[i]
        source_slice = xm_list[i - window_size:i]
        if not (len([c for c in combinations(source_slice, combination_size) if sum(c) == target])):
            return target, i + 1


if __name__ == "__main__":
    xmas_list = parse_xmas_list("data/day09.txt")
    first_mismatch = first_non_sum(xmas_list)
