...
class Program:
    ...
    def execute(self, stop_function, max_iteration=1000):
        ...
        return self.accumulator, self.program_counter, fall_through
...
def changeable_instructions(pgm):
    return [i for i in range(len(pgm)) if pgm[i][0] in ['jmp', 'nop']]


if __name__ == "__main__":
    pgm = fetch_instructions("data/day08.txt")
    for i in changeable_instructions(pgm):
        print(f"Considering change at position {i}")
        new_pgm = pgm.copy()
        opc, arg = new_pgm[i]
        if opc == 'jmp':
            new_pgm[i] = ('nop', arg)
        ...
        executor = Program(new_pgm)
        accum, next_instruction, fall_through = executor.execute(stop_on_loop, _MAX_ITERATIONS)
        if fall_through:
            print(f"Changed instruction at index {i} - original {pgm[i]}")
            print(
